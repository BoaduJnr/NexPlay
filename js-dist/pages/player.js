...
var PlayerPage = function () {
  ...
  var _render = function render(container) {
    ...
    try {
      ...
      _seriesDetails = null;
      try {
        _autoplayEnabled = localStorage.getItem('np_pref_autoplay') === '1';
      } catch (e) {}
      _availableQualities = [];
  ...
  var _downloadHLS = function _downloadHLS() {
    try {
      ...
      function parseM3U8(text, origBase) {
        ...
        return {
          isMaster: isMaster,
          isLive: isLive && !isMaster,
          bestProxyUrl: bestProxyUrl,
          bestOrigUrl: bestOrigUrl,
          segments: segments
        };
      }
      ...
      return Promise.resolve(_catch(function () {
        ...
        return Promise.resolve(fetch(proxyUrl(origUrl), {
          signal: signal
        })).then(function (resp) {
          if (!resp.ok) throw new Error('Playlist fetch failed (' + resp.status + ')');
          return Promise.resolve(resp.text()).then(function (m3u8Text) {
            var _exit = false;
            function _temp13(_result2) {
              if (_exit) return _result2;
              function _temp11() {
                var _exit2 = false;
                function _temp1(_result4) {
                  ...
                  function _temp9() {
                    ...
                    return Promise.resolve(mp4WriteQueue).then(function () {
                      var _exit3 = false;
                      function _temp7(_result5) {
                        ...
                        function _temp5() {
                          ...
                          if (typeof App !== 'undefined') App.showToast('Download complete!' + (failed ? ' (' + failed + ' segments failed)' : ''));
                          setTimeout(function () {
                        ...
                        var _temp4 = function () {
                          if (writable) {
                            ...
                            var totalLen = mp4Chunks.reduce(function (s, c) {
                              return s + c.byteLength;
                            }, 0);
                            var output = new Uint8Array(totalLen);
                            var off = 0;
                            mp4Chunks.forEach(function (c) {
                              output.set(c, off);
                              off += c.byteLength;
                            });
                            var dlBlob = new Blob([output], {
                              type: 'video/mp4'
                            });
                          ...
                          var _temp2 = function () {
                            if (writable) {
                              var _temp14 = _catch(function () {
                                return Promise.resolve(writable.abort()).then(function () {});
                              }, function () {});
                  ...
                  tx.on('data', function (segment) {
                    ...
                    if (writable) {
                      ...
                      pieces.forEach(function (p) {
                        mp4WriteQueue = mp4WriteQueue.then(function () {
                  ...
                  var _temp8 = _for(function () {
                    return !_interrupt && i < segs.length;
                  }, function () {
                    return !!(i += BATCH);
                  }, function () {
                    if (signal.aborted) {
  ...
  var SOURCES = [{
    ...
    movieUrl: function movieUrl(id) {
      return "https://www.vidsrc.wtf/1/movie/".concat(id);
    },
    tvUrl: function tvUrl(id, s, e) {
      return "https://www.vidsrc.wtf/1/tv/".concat(id, "/").concat(s, "/").concat(e);
    }
  }, {
    id: 'videasy',
    label: 'Videasy',
  ...
  var _wtClient = null; // active WebTorrent client — destroyed on stream switch
  var _currentStreamUrl = null; // original (un-proxied) stream URL — used by download
  var _dlXhr = null; // active download XHR — cancelled on player close
  var _hlsDownloadController = null; // AbortController for HLS segment downloads
  ...
  function _updateBufferFill(video, playPct) {
    ...
    var buffEnd = 0;
    try {
      buffEnd = video.buffered.end(video.buffered.length - 1);
    } catch (e) {
      return;
    }
  ...
  function _showAutoplayCountdown(onPlay) {
    ...
    cancelBtn.setAttribute('data-nav', '');
  ...
  function _autoplayNext() {
    ...
    var seasons = _seriesDetails ? (_seriesDetails.seasons || []).filter(function (s) {
      return s.season_number > 0;
    }) : [];
    var nextSeason = null;
    for (var j = 0; j < seasons.length; j++) {
      if (seasons[j].season_number > _currentSeason) {
        nextSeason = seasons[j];
        break;
      }
    }
  ...
  function startAutoHide() {
    ...
    _keyListener2 = function _keyListener(e) {
      ...
      if (k === Config.KEYS.ENTER || k === 13) {
        ...
        var isCtrl = focused && (focused.classList.contains('ctrl-btn') || focused.classList.contains('pcb-btn') || focused.classList.contains('player-settings-btn') || focused.classList.contains('ps-close-btn') || focused.classList.contains('ps-tv-opt-btn') || focused.classList.contains('acct-connect-btn') || focused.tagName === 'BUTTON');
  ...
  function _ensureWebTorrent() {
    ...
    return new Promise(function (resolve) {
      ...
      s.onload = function () {
        resolve(typeof WebTorrent !== 'undefined');
      };
      s.onerror = function () {
        resolve(false);
      };
  ...
  function _startWebTorrent(magnetURI) {
    ...
    _wtClient.add(magnetURI, {
      ...
      var sorted = torrent.files.slice().sort(function (a, b) {
        return b.length - a.length;
      });
      var file = sorted.find(function (f) {
        return /\.mp4$/i.test(f.name) && !f.name.includes('.ia.');
      }) || sorted.find(function (f) {
        return /\.(mp4|webm)$/i.test(f.name);
      });
      if (!file) {
        clearTimeout(watchdog);
      ...
      file.renderTo(video, function (err) {
        ...
        _progressInterval = setInterval(function () {
          ...
          if (pos > 10000 && typeof NexPlayDB !== 'undefined') {
            NexPlayDB.saveProgress(_params.id, _params.type || 'movie', _titleCache, _posterCache, Math.round(pos), Math.round(dur), _currentSeason, _currentEpisode);
          }
        }, 3000);
      });
    });
  ...
  function _fetchWatchers(movieId, wListEl, wCountEl) {
    ...
    xhr.onload = function () {
      ...
      var watchers;
      try {
        watchers = JSON.parse(xhr.responseText);
      } catch (e) {
        return;
      }
      ...
      wListEl.innerHTML = watchers.map(function (w) {
        return '<div class="ps-watching-item">' + '<span class="ps-watching-av">' + (w.picture ? '<img src="' + _escHtmlP(w.picture) + '" class="ps-watching-avatar" onerror="this.style.display=\'none\'">' : '<span class="ps-watching-avatar-ph">' + (w.name || '?')[0].toUpperCase() + '</span>') + '<span class="online-dot ps-watching-online"></span>' + '</span>' + '<span class="ps-watching-name">' + _escHtmlP(w.name || 'Someone') + '</span>' + '</div>';
      }).join('');
    };
  ...
  function _makeDropdown(opts, selectedIdx, placeholder, onSelect, up) {
    ...
    trigger.innerHTML = (current ? current.html || current.label : placeholder) + '<svg class="ps-dd-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="12" height="12"><polyline points="6 9 12 15 18 9"/></svg>';
    ...
    opts.forEach(function (opt, i) {
      ...
      item.innerHTML = (opt.html || opt.label) + (i === selectedIdx ? '<svg class="ps-dd-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="13" height="13"><polyline points="20 6 9 17 4 12"/></svg>' : '');
      item.addEventListener('click', function (e) {
        e.stopPropagation();
        trigger.innerHTML = (opt.html || opt.label) + '<svg class="ps-dd-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="12" height="12"><polyline points="6 9 12 15 18 9"/></svg>';
        list.querySelectorAll('.ps-dd-item').forEach(function (it, ii) {
          ...
          if (ii === i && !ck) it.innerHTML += '<svg class="ps-dd-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="13" height="13"><polyline points="20 6 9 17 4 12"/></svg>';else if (ii !== i && ck) ck.parentNode.removeChild(ck);
        });
    ...
    trigger.addEventListener('click', function (e) {
      ...
      if (panel) panel.querySelectorAll('.ps-dd-list').forEach(function (l) {
        l.classList.add('hidden');
      });
      document.querySelectorAll('.ps-dd-trigger').forEach(function (t) {
        t.classList.remove('ps-dd-open');
      });
      if (!isOpen) {
  ...
  function _renderSettingsPanel(panel) {
    ...
    if (_availableQualities && _availableQualities.length) {
      ...
      if (isTV) {
        ...
        qToggleBtn.setAttribute('data-nav', '');
        qToggleBtn.setAttribute('tabindex', '0');
        qToggleBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>' + ' Quality' + '<span class="ps-stgl-cur" style="margin-left:6px;font-size:11px;opacity:0.55;font-weight:400">' + _availableQualities[qSelIdx].label + '</span>' + '<svg class="ps-stgl-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="12" height="12"' + ' style="margin-left:auto;flex-shrink:0;-webkit-transform:rotate(-90deg);transform:rotate(-90deg)"><polyline points="6 9 12 15 18 9"/></svg>';
    ...
    if (isTV) {
      ...
      sToggleBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M7 15h4M13 15h4M7 11h2M11 11h6"/></svg>' + ' Subtitles' + '<svg class="ps-stgl-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="12" height="12"' + ' style="margin-left:auto;flex-shrink:0;-webkit-transform:rotate(-90deg);transform:rotate(-90deg)"><polyline points="6 9 12 15 18 9"/></svg>';
    ...
    if (_params.type === 'tv') {
      ...
      apRow.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13" style="flex-shrink:0;"><polygon points="5 3 19 12 5 21 5 3"/></svg>' + '<span style="margin-left:6px;-webkit-flex:1;flex:1;">Autoplay</span>';
      ...
      apBtn.addEventListener('click', function () {
        ...
        try {
          localStorage.setItem('np_pref_autoplay', _autoplayEnabled ? '1' : '0');
        } catch (e) {}
      });
    ...
    if (_currentStreamUrl && typeof webapis === 'undefined') {
      ...
      if (_availableQualities && _availableQualities.length) {
        var dOpts = _availableQualities.map(function (q) {
          return {
            label: q.label
          };
        });
        var dDD = _makeDropdown(dOpts, _dlQualityIdx, 'Auto', function (opt, i) {
          _dlQualityIdx = i;
        });
  ...
  function _openSubPicker() {
    ...
    if (document.getElementById('player-sub-picker')) {
      _closePicker();
      return;
    }
    ...
    SubtitleClient.getLanguages(_params.id, _params.type || 'movie', _currentSeason, _currentEpisode).then(function (langs) {
      ...
      offOpt.innerHTML = '<span class="sub-lang-off-icon">✕</span><span class="sub-lang-name">Off</span>' + (!_subActiveLang ? '<span class="sub-lang-check">✓</span>' : '');
      offOpt.addEventListener('click', function () {
        ...
        if (btn) {
          ...
          btn.querySelector('.cc-label').textContent = 'CC';
        }
        _closePicker();
      });
      ...
      langs.forEach(function (l) {
        ...
        var flagHtml = l.flagUrl ? '<img class="sub-lang-flag" src="' + l.flagUrl + '" onerror="this.style.display=\'none\'">' : '<span class="sub-lang-flag-placeholder"></span>';
        opt.innerHTML = flagHtml + '<span class="sub-lang-name">' + (l.display || l.language) + '</span>' + (isActive ? '<span class="sub-lang-check">✓</span>' : '<span class="sub-lang-loading" style="display:none">…</span>');
        opt.addEventListener('click', function () {
          var loadingEl = opt.querySelector('.sub-lang-loading');
          if (loadingEl) loadingEl.style.display = '';
          SubtitleClient.loadLanguage(_params.id, _params.type || 'movie', l.language, _currentSeason, _currentEpisode).then(function (sub) {
            if (!sub) {
              if (typeof App !== 'undefined') App.showToast('Subtitle not available');
              _closePicker();
              return;
            }
            _subActiveLang = l.language;
  ...
  function _resetDownloadUI() {
    ...
    if (ico) {
      ico.innerHTML = '<path d="M12 5v14M5 12l7 7 7-7"/>';
    }
    if (fill) {
      fill.style.width = '0%';
    }
  ...
  function stopAvPlay() {
    ...
    if (_hlsDownloadController) {
      try {
        _hlsDownloadController.abort();
      } catch (e) {}
      _hlsDownloadController = null;
    }
    _resetDownloadUI();
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, function () {
        ...
        if (typeof SubtitleClient !== 'undefined' && typeof webapis === 'undefined' && !_subActiveLang) {
          SubtitleClient.getLanguages(_params.id, _params.type || 'movie', _currentSeason, _currentEpisode).then(function (langs) {
            ...
            var en = langs.find(function (l) {
              return l.language === 'en' || l.language === 'eng';
            });
            if (!en) return;
            SubtitleClient.loadLanguage(_params.id, _params.type || 'movie', en.language, _currentSeason, _currentEpisode).then(function (sub) {
