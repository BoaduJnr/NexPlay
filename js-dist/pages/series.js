...
var SeriesPage = function () {
    ...
    try {
      ...
      var _temp3 = _catch(function () {
        return Promise.resolve(fetchShows(_page)).then(function (data) {
          ...
          UX.fetchInAppRatings(data.results.map(function (s) {
            return String(s.id);
          }));
          if (replace) {
  ...
  function bindRemoteKeys() {
    _keyHandler = function _keyHandler(e) {
      ...
      var rating = focused.dataset.showRating || '0';
      if (e.keyCode === Config.KEYS.RED) {
        e.preventDefault();
        var added = NexPlayDB.toggleFavourite(showId, 'tv', title, poster, rating);
        ...
        e.preventDefault();
        var _added = NexPlayDB.toggleWatchlist(showId, 'tv', title, poster, rating);
