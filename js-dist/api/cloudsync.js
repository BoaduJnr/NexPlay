...
var CloudSync = function () {
  ...
  function syncUp() {
    ...
    try {
      statusHidden = localStorage.getItem('np_status_hidden') === '1';
    } catch (e) {}
      ...
      xhr.onload = function () {
        ...
        if (data.favourites !== undefined && data.favourites !== null) {
          try {
            ...
            var localNewFavs = localFavs.filter(function (f) {
              return (f.addedAt || 0) > serverTs;
            });
        ...
        if (data.watchlist !== undefined && data.watchlist !== null) {
          try {
            ...
            var localNewWl = localWl.filter(function (f) {
              return (f.addedAt || 0) > serverTs;
            });
        ...
        if (data.history && Array.isArray(data.history)) {
          try {
            ...
            var localNewHistory = localHistory.filter(function (h) {
              return (h.watchedAt || 0) > serverTs;
            });
            var histMap = {};
            data.history.forEach(function (h) {
              histMap[h.id + '|' + h.type + '|' + (h.season || '') + '|' + (h.episode || '')] = h;
            });
            ...
            var mergedHistory = Object.keys(histMap).map(function (k) {
              return histMap[k];
            }).sort(function (a, b) {
        ...
        if (data.progress && _typeof(data.progress) === 'object') {
          try {
        ...
        if (typeof data.statusHidden === 'boolean') {
          try {
            localStorage.setItem('np_status_hidden', data.statusHidden ? '1' : '0');
          } catch (e) {}
        }
