...
var StreamResolver = function () {
  ...
  // ── Videasy ────────────────────────────────────────────
  var VIDEASY_ENDPOINTS = ['https://api.videasy.net/mb-flix/sources-with-title', 'https://api.videasy.net/cdn/sources-with-title', 'https://api.videasy.net/superflix/sources-with-title', 'https://api.videasy.net/lamovie/sources-with-title'];
  ...
  function resolveVideasy(tmdbId, type, title, year, season, episode) {
    var qs = 'title=' + encodeURIComponent(title || '') + '&mediaType=' + (type === 'tv' ? 'show' : 'movie') + '&tmdbId=' + tmdbId + '&imdbId=' + '&language=english';
    ...
    function tryEndpoint(idx) {
      ...
      return xhrGet(proxyApiUrl, 12000).then(function (hexBlob) {
        ...
        return xhrPost(DEC_URL, JSON.stringify({
          ...
          var best = sources.find(function (s) {
            return s.quality === '1080p';
          }) || sources.find(function (s) {
            return s.quality === '720p';
          }) || sources[0];
  ...
  function resolveVidrock(tmdbId, type, season, episode) {
    ...
    return encryptVidrock(itemId).then(function (token) {
      ...
      return xhrGet(apiUrl, 10000, {
        ...
        var best = hlsUrls.find(function (s) {
          return s.label === '1080p';
        }) || hlsUrls.find(function (s) {
          return s.label === '720p';
        }) || hlsUrls[0];
  ...
  function resolveMovie(tmdbId, title, year) {
    ...
    return resolveVideasy(tmdbId, 'movie', title, year, null, null).then(function (result) {
      ...
      return tryScrapeUrls(['https://vidsrc.xyz/embed/movie?tmdb=' + tmdbId, 'https://vidsrc.me/embed/movie?tmdb=' + tmdbId, 'https://multiembed.mov/?video_id=' + tmdbId + '&tmdb=1']);
    }).catch(function (e) {
