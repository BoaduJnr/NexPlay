...
(function() {
  ...
  function getPanel() {
    if (!_panel) {
      ...
      _panel.style.cssText = 'position:fixed;bottom:0;left:0;width:100%;max-height:360px;' +
        'overflow-y:auto;background:rgba(0,0,0,0.92);color:#0f0;font-size:18px;' +
        'font-family:monospace;padding:12px;z-index:99998;display:none;';
  ...
  function renderPanel() {
    ...
    p.innerHTML = _logs.map(function(l) {
      ...
      return '<div style="color:' + c + ';border-bottom:1px solid #222;padding:2px 0;">' +
        l.replace(/</g,'&lt;') + '</div>';
    }).join('');
  ...
  console.log   = function() { addLog('LOG',  arguments); _orig.log.apply(console, arguments); };
  console.error = function() { addLog('ERR',  arguments); _orig.error.apply(console, arguments); };
  console.warn  = function() { addLog('WARN', arguments); _orig.warn.apply(console, arguments); };
  ...
  document.addEventListener('keydown', function(e) {
    var toggle = false;
    if (e.keyCode === 405) {           // Yellow
      toggle = true;
    } else if (e.keyCode === 457) {    // Info — triple-press fallback
      _infoCount++;
      clearTimeout(_infoTimer);
      _infoTimer = setTimeout(function() { _infoCount = 0; }, 800);
      if (_infoCount >= 3) { _infoCount = 0; toggle = true; }
    }
    if (toggle) {
      _visible = !_visible;
      var p = getPanel();
