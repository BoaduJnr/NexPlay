﻿﻿﻿const PlayerPage = (() => {
      ...
      webapis.avplay.prepareAsync(
        function() {
          ...
          if (typeof NexPlayDB !== 'undefined') {
            ...
            if (saved && saved.position > 10000) { // only restore if > 10s
              try {
  ...
  function renderQualityDropdown() {
    ...
    TVDropdown.mount('quality-dd', function(val) {
      ...
      if (!q) return;
      var headers = { 'Referer': 'https://player.videasy.net/', 'Origin': 'https://player.videasy.net' };
      stopAvPlay();
      setPlayerStatus('Loading...');
  ...
  async function render(container, params = {}) {
    ...
    modal.innerHTML = `
      <div class="player-header" style="${isTV ? 'margin-right:300px;' : ''}">
      ...
      <div class="player-cbar" id="player-cbar" style="${isTV ? 'right:300px;' : ''}">
      ...
      <div class="player-info-bar" style="${isTV ? 'margin-right:300px;' : ''}">
    ...
    TVDropdown.mount('quality-dd', function(val) {
      ...
      if (!q) return;
      var hdrs = { 'Referer': 'https://player.videasy.net/', 'Origin': 'https://player.videasy.net' };
      stopAvPlay(); setPlayerStatus('Loading...'); playWithUrl(q.url, hdrs);
    });
