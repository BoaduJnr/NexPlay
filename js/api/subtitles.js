...
var SubtitleClient = (function() {
  ...
  function cacheRead(key) {
    try { var raw = localStorage.getItem(key); if (!raw) return null; var obj = JSON.parse(raw); return Date.now() - obj.ts > CACHE_TTL ? null : obj.data; }
    catch (e) { return null; }
  }
  ...
  function getImdbId(tmdbId) {
    ...
    return fetch(Config.TMDB_BASE + '/movie/' + tmdbId + '/external_ids?api_key=' + Config.TMDB_KEY)
      ...
      .then(function(d) { var id = d && d.imdb_id ? d.imdb_id : null; if (id) cacheWrite(ck, id); return id; })
  ...
  function fetchAll(tmdbId, type, season, episode) {
    ...
    return idPromise.then(function(searchId) {
      if (!searchId) return [];
      var url = BASE_URL + '/search?id=' + encodeURIComponent(searchId)
      ...
      return fetch(url, { headers: { 'Accept': 'application/json' } })
        ...
        .then(function(arr) {
          ...
          var filtered = arr.filter(function(s) { return s && s.url && s.language; });
  ...
  function getLanguages(tmdbId, type, season, episode) {
    return fetchAll(tmdbId, type, season, episode).then(function(all) {
      ...
      all.forEach(function(s) {
        var lc = s.language;
        if (!map[lc]) map[lc] = { language: lc, display: s.display || lc, flagUrl: s.flagUrl || '', count: 0 };
        map[lc].count += (s.downloadCount || 1);
      });
      var langs = Object.keys(map).map(function(k) { return map[k]; });
  ...
  function loadLanguage(tmdbId, type, language, season, episode) {
    return fetchAll(tmdbId, type, season, episode).then(function(all) {
      ...
      var forLang = all.filter(function(s) { return s.language === language; });
      ...
      return fetch(best.url)
        .then(function(r) { return r.ok ? r.text() : null; })
        .then(function(text) {
          if (!text) return null;
          // SRT → VTT conversion
          var vtt = text.trim().startsWith('WEBVTT') ? text
  ...
  function applySubtitle(videoEl, sub) {
    ...
    if (old) { try { URL.revokeObjectURL(old.src); } catch(e) {} old.parentNode.removeChild(old); }
  ...
  function applyToVideo(videoEl, tmdbId, type, lang, season, episode) {
    ...
    return getLanguages(tmdbId, type, season, episode)
      .then(function(langs) { return langs.length > 0; })
