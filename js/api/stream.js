...
var StreamResolver = (function() {
  ...
  var VIDEASY_ENDPOINTS = [
    'https://api.videasy.to/mb-flix/sources-with-title',
    'https://api.videasy.to/cdn/sources-with-title',
    'https://api.videasy.to/hdmovie/sources-with-title',
    'https://api.videasy.to/lamovie/sources-with-title',
  ];
  ...
  function resolveVideasy(tmdbId, type, title, year, season, episode, imdbId) {
    ...
    var imdbPromise = imdbId
      ...
      : fetch(Config.TMDB_BASE + '/' + (type === 'tv' ? 'tv' : 'movie') + '/' + tmdbId + '/external_ids?api_key=' + Config.TMDB_KEY)
          .then(function(r) { return r.ok ? r.json() : null; })
          .then(function(d) { return d && d.imdb_id ? d.imdb_id : ''; })
  ...
  function resolveVidrock(tmdbId, type, season, episode) {
    ...
    return encryptVidrock(itemId).then(function(token) {
      ...
      return xhrGet(apiUrl, 12000, vdrHeaders)
        .then(parseVidrock)
        .catch(function() {
          ...
          return xhrGet(proxyUrl, 12000).then(parseVidrock).catch(function() { return null; });
        });
    }).catch(function(e) {
  ...
  function _parseBencode(bytes, pos) {
    ...
    if (ch === 108) {
      var list = []; pos++;
      while (pos < bytes.length && bytes[pos] !== 101) { var r = _parseBencode(bytes, pos); list.push(r[0]); pos = r[1]; }
      return [list, pos + 1];
    }
    // Dict: d...e
    if (ch === 100) {
      ...
      while (pos < bytes.length && bytes[pos] !== 101) {
        var k = _parseBencode(bytes, pos); pos = k[1];
        var v = _parseBencode(bytes, pos); pos = v[1];
        dict[k[0]] = v[0];
      }
      return [dict, pos + 1];
    }
  ...
  function _parseTorrentInfo(bytes) {
    try {
      ...
      seeds = seeds.filter(function(u) { return typeof u === 'string' && u.indexOf('http') === 0; });
      // Find first MP4 path
      var mp4Path = null;
      if (Array.isArray(files)) {
        for (var i = 0; i < files.length; i++) {
          ...
          var ps = pathArr.join('/');
          if (/\.mp4$/i.test(ps)) { mp4Path = ps; break; }
        }
      } else if (/\.mp4$/i.test(name)) {
        mp4Path = name;
      }
      return { seeds: seeds, name: name, mp4Path: mp4Path };
    } catch(e) { return null; }
  }
  ...
  function _doResolveTorrentTV(tmdbId) {
    ...
    return xhrGet(Config.TMDB_BASE + '/movie/' + tmdbId + '/external_ids?api_key=' + Config.TMDB_KEY, 8000)
      .then(function(text) {
        var data; try { data = JSON.parse(text); } catch(e) { return null; }
        var imdbId = data && data.imdb_id;
        if (!imdbId) { console.log('[StreamResolver] TV torrent: no IMDB ID'); return null; }
        ...
        return xhrGet(PROXY_URL + '/?url=' + encodeURIComponent(ytsUrl), 12000)
          .then(function(text2) {
            var yts; try { yts = JSON.parse(text2); } catch(e) { return null; }
            var movie = yts && yts.data && yts.data.movie;
            if (!movie || !movie.torrents || !movie.torrents.length) {
              console.log('[StreamResolver] TV torrent: no YTS entry'); return null;
            }
            var torrents = movie.torrents;
            var t = torrents.find(function(t) { return t.quality === '1080p' && t.type === 'bluray'; })
                 || torrents.find(function(t) { return t.quality === '1080p'; })
                 || torrents.find(function(t) { return t.quality === '720p'; })
                 || torrents[0];
            ...
            return xhrGetBinary(torrentProxyUrl, 15000)
              .then(function(ab) {
                if (!ab) { console.log('[StreamResolver] TV torrent: failed to fetch .torrent'); return null; }
                var info = _parseTorrentInfo(new Uint8Array(ab));
                if (!info || !info.seeds.length) { console.log('[StreamResolver] TV torrent: no web seeds'); return null; }
                ...
                if (!videoUrl) { console.log('[StreamResolver] TV torrent: cannot build URL'); return null; }

                console.log('[StreamResolver] TV torrent direct:', t.quality, videoUrl.slice(0, 80));
                return { url: videoUrl, headers: {}, qualities: [{ label: t.quality, url: videoUrl }] };
              });
          });
      })
      .catch(function(e) {
  ...
  function _doResolveTorrent(tmdbId) {
    ...
    // Step 1: TMDB → IMDB ID
    return fetch(Config.TMDB_BASE + '/movie/' + tmdbId + '/external_ids?api_key=' + Config.TMDB_KEY)
      .then(function(r) { return r.ok ? r.json() : null; })
      .then(function(data) {
        if (!data) return null;
        var imdbId = data.imdb_id;
        if (!imdbId) { console.log('[StreamResolver] no IMDB ID for tmdb=' + tmdbId); return null; }

        // Step 2: YTS API via proxy.
        // yts.mx is blocked by Cloudflare for CF Worker IPs — use yts.lt mirror instead.
        var ytsUrl = 'https://yts.lt/api/v2/movie_details.json?imdb_id=' + imdbId + '&with_torrents=true';
        return fetch(PROXY_URL + '/?url=' + encodeURIComponent(ytsUrl))
          .then(function(r) { return r.ok ? r.json() : null; })
          .then(function(yts) {
            var movie = yts && yts.data && yts.data.movie;
            if (!movie || !movie.torrents || !movie.torrents.length) {
            ...
            var withSeeds = torrents.filter(function(t) { return t.seeds > 0; });
            var isWeb = function(t) { return /web/i.test(t.type); };
            var t = withSeeds.find(function(t) { return t.quality === '1080p' && isWeb(t); })
                 || withSeeds.find(function(t) { return t.quality === '720p'  && isWeb(t); })
                 || withSeeds.find(function(t) { return t.quality === '1080p'; })
                 || withSeeds.find(function(t) { return t.quality === '720p'; })
                 || withSeeds[0];
            ...
            return { type: 'torrent', magnetURI: torrentFileUrl, hash: t.hash, quality: t.quality, title: movie.title_long };
          });
      })
      .catch(function(e) {
  ...
  function resolveMovie(tmdbId, title, year) {
    ...
    var step2 = function() { return resolveVideasy(tmdbId, 'movie', title, year, null, null); };
  ...
  function resolveTVEpisode(tmdbId, title, season, episode) {
    ...
    var step2 = function() { return resolveVideasy(tmdbId, 'tv', title, null, season, episode); };
  ...
  return { resolveMovie: resolveMovie, resolveTVEpisode: resolveTVEpisode, scrapeEmbed: scrapeEmbed, resolveTorrent: resolveTorrent };
