...
var EPGClient = (function() {
  ...
  function parseWorkerJSON(json, targetId) {
    ...
    function stripFeed(ch) { return (ch || '').split('@')[0].toLowerCase(); }
    ...
    programs.forEach(function(p) {
      ...
      result.push({ start: s, stop: e2, title: title, desc: desc });
    });
  ...
  function loadGuides() {
    ...
    if (cached) { _channelMap = cached; return Promise.resolve(_channelMap); }

    _guidesPromise = fetch(GUIDES_URL)
      .then(function(r) {
      ...
      .then(function(guides) {
        ...
        guides.forEach(function(g) {
          ...
          var jsonSrc = sources.find(function(s) { return s.format === 'JSON'; });
  ...
  function fetchProgrammes(tvgId) {
    ...
    if (cached) { _programmes[idKey] = cached; return Promise.resolve(cached); }

    _pending[idKey] = loadGuides()
      .then(function(map) {
        var urls = map[idKey] || [];
