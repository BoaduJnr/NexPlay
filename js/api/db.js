...
var NexPlayDB = (function() {
  ...
  function saveProgress(id, type, title, poster, positionMs, durationMs, season, episode) {
    ...
    progress[key] = {
      id: String(id), type: type, title: title, poster: poster || '',
      position: positionMs, duration: durationMs || 0,
  ...
  function getContinueWatching(limit) {
    ...
    return Object.values(progress)
      .filter(function(p) { return p.duration > 0 && p.position < p.duration * 0.95; })
  ...
  function addToHistory(id, type, title, poster, season, episode) {
    ...
    history = history.filter(function(h) {
      return !(h.id === String(id) && h.type === type &&
               h.season === (season||null) && h.episode === (episode||null));
    });
    history.unshift({
      id: String(id), type: type, title: title, poster: poster || '',
      season: season || null, episode: episode || null,
  ...
  function clearHistory() { save(K.HISTORY, []); }
  ...
  function createPlaylist(name) {
    ...
    var playlist = { id: genId(), name: name, items: [], createdAt: Date.now() };
    playlists.unshift(playlist);
    save(K.PLAYLISTS, playlists);
    return playlist;
  }

  function getPlaylists() { return load(K.PLAYLISTS) || []; }

  function addToPlaylist(playlistId, id, type, title, poster) {
    var playlists = load(K.PLAYLISTS) || [];
    var pl = playlists.find(function(p) { return p.id === playlistId; });
    if (!pl) return false;
    var exists = pl.items.some(function(i) { return i.id === String(id) && i.type === type; });
    if (!exists) pl.items.unshift({ id: String(id), type: type, title: title, poster: poster || '' });
    save(K.PLAYLISTS, playlists);
    return true;
  }
  ...
  function removeFromPlaylist(playlistId, id, type) {
    ...
    var pl = playlists.find(function(p) { return p.id === playlistId; });
    if (!pl) return false;
    pl.items = pl.items.filter(function(i) { return !(i.id === String(id) && i.type === type); });
    save(K.PLAYLISTS, playlists);
    return true;
  }

  function deletePlaylist(playlistId) {
    var playlists = (load(K.PLAYLISTS) || []).filter(function(p) { return p.id !== playlistId; });
    save(K.PLAYLISTS, playlists);
  }
  ...
  function isInPlaylist(playlistId, id, type) {
    var playlists = load(K.PLAYLISTS) || [];
    var pl = playlists.find(function(p) { return p.id === playlistId; });
    return pl ? pl.items.some(function(i) { return i.id === String(id) && i.type === type; }) : false;
  }
  ...
  function toggleFavourite(id, type, title, poster, rating) {
    ...
    if (idx >= 0) { favs.splice(idx, 1); save(K.FAVOURITES, favs); }
    else { favs.unshift({ id: String(id), type: type, title: title, poster: poster || '', rating: parseFloat(rating) || 0, addedAt: Date.now() }); save(K.FAVOURITES, favs); }
    if (typeof CloudSync !== 'undefined') CloudSync.queueSyncUp();
    return idx < 0;
  }
  ...
  function toggleWatchlist(id, type, title, poster, rating) {
    ...
    if (idx >= 0) { wl.splice(idx, 1); save(K.WATCHLIST, wl); }
    else { wl.unshift({ id: String(id), type: type, title: title, poster: poster || '', rating: parseFloat(rating) || 0, addedAt: Date.now() }); save(K.WATCHLIST, wl); }
    if (typeof CloudSync !== 'undefined') CloudSync.queueSyncUp();
    return idx < 0;
  }
